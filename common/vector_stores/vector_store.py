    ...
    def __init__(self, embeddings_provider ,store_name = "FAISS",index=None):
        ...
        match store_name:
            case "FAISS":
                ...
                self._index = faiss.IndexFlatL2(len(self._embeddings_provider.embeddings.embed_query("hello world")))
                self._store = FAISS_VECTOR_STORE(doc_store = doc_store, embedding_function = self._embeddings_provider, index = self._index, index_to_docstore_id = {})
...
class FAISS_VECTOR_STORE(FAISS):
    ...
    def __init__(self,**kwargs):
        ...
        self._store = super().__init__(docstore = self._doc_store, embedding_function = self._embedding_function.embeddings, index = self._index, index_to_docstore_id = self._index_to_docstore_id)
